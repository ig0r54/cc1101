...
uint8_t CC1100::begin(volatile uint8_t &My_addr)
{
    ...
    pinMode(GDO0, PINMODE_INPUT);                 //setup AVR GPIO ports
    pinMode(GDO2, PINMODE_INPUT);
    ...
    do                                                          //sent package out with retries
    {
        ...
        while (ackWaitCounter < ACK_TIMEOUT )                   //wait for an acknowledge
        {
            if (packet_available() == TRUE)                     //if RF package received check package acknowge
            {
                ...
                delayMicroseconds(1000);                                       //delay to give receiver time
            }
        }
...
uint8_t CC1100::wait_for_packet(uint16_t milliseconds)
{
    for(uint16_t i = 0; i < milliseconds; i++)
        {
            delayMicroseconds(1000);                 //delay till system has data available
            if (packet_available())
            {
                return TRUE;
            }
...
void CC1100::spi_begin(void)
{
     int x = 0;
     //printf ("init SPI bus... ");
     if ((x = wiringXSPISetup (0, 8000000)) < 0)  //4MHz SPI speed
     {
          if(debug_level > 0){
          printf ("ERROR: wiringXSPISetup failed!\r\n");
          }
     }
     else{
